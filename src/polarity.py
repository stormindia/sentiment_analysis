...
# code for determining the polarity and POS characteristics
...
def loadSentiFull(filename):
    ...
    line=fi.readline() # to skip the first header line
    line=fi.readline()
    print "Loading..."
...
def polarity(tweet,sentDict): # polarity aggregate of a tweet from sentiWordnet dict
    ...
    for w in tweet.split():
        ...
        if features.hashTest(w) and w[1:] in sentDict.keys():
            pos=pos+2*sentDict[w[1:]][1] #imp. - more weight for hashed words
...
def posPolarity(tweet,sentDict): # polarity aggregate of a tweet from sentiWordnet dict
    ...
    for z in tags:
        ...
        if features.hashTest(list(w)[0]) and (list(w)[0][1:],list(w)[1]) in sentDict.keys():
            ...
            pos=pos+2*sentDict[(list(w)[0][1:],list(w)[1])][0] # imp. - more weight for hashed words
