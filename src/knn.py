...
def mapTweet(tweet,afinn,emoDict,positive,negative,neutral,slangs):
    ...
    out.append(p)
    out.append(float(features.emoticonScore(line,emoDict))) # emo total score be careful to modify weights
...
#OPTIMIZE PRECISION
def optimizeKNN(X,Y,n): # n number of features : unigram is one feature
    print "Optimizing features weights..."
    best_weights=[]
    best_accuracy=0.0
    w=[0.2*i for i in range (1,6)] # w different possible weights for a single feature
    W=[]
...
def predict(tweet,model): # test a tweet against a built model
    ...
    z=z[0].tolist()
    return model.predict([z]).tolist() # create list
