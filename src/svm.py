...
def mapTweet(tweet,afinn,emoDict,positive,negative,neutral,slangs):
    ...
    out.append(p)
    out.append(float(features.emoticonScore(line,emoDict))) # rem to modify weights
...
def predict(tweet,model): # test a tweet against a built model
    ...
    z=z[0].tolist()
    return model.predict([z]).tolist()[0] # creating list

def predictFile(filename,svm_model): # load test file
    f=open(filename,'r')
...
def loadTest(filename): # load test file
    f=open(filename,'r')
    line=f.readline()
    labels=[]
    vectors=[]
