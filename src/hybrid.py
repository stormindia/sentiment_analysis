...
print "Please wait while dictionary is initialized"
...
print "Please wait while unigrams vector is being buitl"
positive=ngramGenerator.mostFreqList('../data/used/positive1.csv',UNIGRAM_SIZE)
...
#
def modiKNN(z): # total negative and positive scores for knn
    l=z[0:len(z)-3]
...
def predictOneKNN(tweet,model,scaler,normalizer): # test a tweet against a built model KNN or SVM not both
    ...
    z=z[0].tolist()
    return float(model.predict([z]).tolist()[0]) # create list
...
def predictTwo(tweet,knn_model,svm_model): # perform two step classification : first KNN then SVM
    z=mapTweet(tweet,afinn,emoticonDict,positive,negative,neutral,slangs) # mapping
...
#def predictFile
def predictFile(filename,knn_model,svm_model): # load test file in the csv format
    f=open(filename,'r')
...
def validateHybrid(X,Y,n,knel,c):
    scores=[] # list of accuracy values for each fold
    for k in range(1,6):
...
print "Model Built .Continue if you want to classify a tweet ? ..."


user_input=raw_input("Write a tweet to test or a file path for bulk classification with Hybrid model. press q to exit\n")
while user_input!='q':
    try:
